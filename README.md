
# Hi there, I'm Sheila 👋

I'm an **Operations & Automation Engineer** passionate about building AI-powered systems that eliminate repetitive work and help teams operate more efficiently.

My background combines Mechanical Engineering, data analytics, workflow automation, and AI. I enjoy designing practical solutions that connect APIs, automate business processes, and turn complex workflows into simple, reliable experiences.

I'm particularly interested in:

* 🤖 AI-powered workflow automation
* ⚙️ Business process optimisation
* 🔗 API integrations
* 📊 Data analytics & operational insights
* ☁️ Cloud-based automation platforms

### Tech Stack

* **Automation:** n8n, Zapier
* **Programming:** Python, SQL
* **AI:** LLM APIs, Prompt Engineering
* **Data & BI:** Tableau, Excel
* **Tools:** Git, GitHub

### What I'm Building

* **RotaChief** – An AI-powered workforce scheduling platform that automates rota generation, attendance tracking, and workforce management.
* Automation workflows that integrate AI models, APIs, and business applications to streamline operations. Check it out here: www.rotachief.com

I'm always interested in learning, building, and collaborating on projects that solve real-world operational challenges through automation and AI.



📫 How to reach me: ibierejohn@gmail.com 


[<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/github.svg' alt='github' height='40'>](https://github.com/sheilairags)  [<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/linkedin.svg' alt='linkedin' height='40'>](https://www.linkedin.com/in/https://www.linkedin.com/in/sheila-john//)  [<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/icloud.svg' alt='website' height='40'>](https://sheilairags.github.io)  [<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/tableau.svg' alt='tableau' height='40'>](https://public.tableau.com/app/profile/sheila.john3044)  


